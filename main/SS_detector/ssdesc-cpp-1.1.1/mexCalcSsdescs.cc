...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	if (nrhs == 3)
	{
		...
		if ((max_dim == 2) || (max_dim == 4) || (max_dim == 5))
		{
			// check ranges
			if (max_dim == 2)
			{
    ...
    const double *img_ptr = img;
    bool isWindow = false;
    const int x0 = calcRect.xfrom - (parms.cor_size-1)/2;
    const int x1 = calcRect.xfrom + (parms.cor_size-1)/2;
    const int y0 = calcRect.yfrom - (parms.cor_size-1)/2;
    const int y1 = calcRect.yfrom + (parms.cor_size-1)/2;
    vector<double> img_local(parms.cor_size*parms.cor_size*img_nchan);
    if (calcRect.xfrom == calcRect.xto && calcRect.yfrom == calcRect.yto) {
        isWindow = true;
        
        for (int n = 0; n < img_nchan; ++n)
            for (int x = x0; x <= x1; ++x)
                for (int y = y0; y <= y1; ++y)
                    img_local[(y - y0) + ((x - x0) + n*parms.cor_size)*parms.cor_size] = img[y + (x + n*img_width)*img_height];
        
        img_width = img_height = parms.cor_size;
        img_ptr = &img_local[0];
    ...
    LARGE_INTEGER _start, _stop, _stop2;
    
    QueryPerformanceFrequency(&_pfFrequency);
    QueryPerformanceCounter(&_start);*/

    if (step > 1 || isWindow)
        ssdesc::calc_ssdescs<double>(img_ptr, img_width, img_height, img_nchan, parms, &ssdescs, calcRect, step);
    else
        ssdesc::calc_ssdescs_alt<double>(img_ptr, img_width, img_height, img_nchan, parms, &ssdescs, calcRect);
    //QueryPerformanceCounter(&_stop);
	ssdesc::prune_normalise_ssdescs<double>(ssdescs, img_width, img_height, parms, &resp, &drawCoords, &salientCoords, &homogeneousCoords, &snnCoords, calcRect, step);
    /*QueryPerformanceCounter(&_stop2);
    if (!isWindow) {
        printf("Duration: %f, then %f\n",
            double(_stop .QuadPart - _start.QuadPart)/double(_pfFrequency.QuadPart),
            double(_stop2.QuadPart - _stop .QuadPart)/double(_pfFrequency.QuadPart)
        );
    }*/
    ...
    int xOff = isWindow ? x0 + 1 : 1;
    int yOff = isWindow ? y0 + 1 : 1;
	...
	for (int i=0; i<(int)drawCoords.size(); ++i)
	{
		...
		drawCoords_out[i*2] = drawCoords[i].x + xOff;
		drawCoords_out[i*2+1] = drawCoords[i].y + yOff;
	}
	vector<ssdesc::coordElem>().swap(drawCoords);
	for (int i=0; i<(int)salientCoords.size(); ++i)
	{
		salientCoords_out[i*2] = salientCoords[i].x + xOff;
		salientCoords_out[i*2+1] = salientCoords[i].y + yOff;
	}
	vector<ssdesc::coordElem>().swap(salientCoords);
	for (int i=0; i<(int)homogeneousCoords.size(); ++i)
	{
		homogeneousCoords_out[i*2] = homogeneousCoords[i].x + xOff;
		homogeneousCoords_out[i*2+1] = homogeneousCoords[i].y + yOff;
	}
	vector<ssdesc::coordElem>().swap(homogeneousCoords);
	for (int i=0; i<(int)snnCoords.size(); ++i)
	{
		snnCoords_out[i*2] = snnCoords[i].x + xOff;
		snnCoords_out[i*2+1] = snnCoords[i].y + yOff;
	}
