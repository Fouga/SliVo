...
namespace ssdesc
{
	...
	template<class fnumtype> void calc_ssdescs(const fnumtype* image, const int image_width,
		...
		vector<fnumtype>* ssdescs, const rectRegion calc_rect = rectRegion(), int step = 1);
	...
	namespace details
	{
        ...
        template<class fnumtype, int nchan, int sz, int ssd_sz> void ssd_compute_specialized(const fnumtype* img,
